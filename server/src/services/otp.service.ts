...
export class OtpService {
   ...
   */
  public static async requestOtp(phone: string, purpose: string = 'AUTH'): Promise<{ success: boolean; message: string }> {
    ...
    // 2. Génération cryptographique aléatoire d'un code OTP à 6 chiffres
    ...
    // 4. Envoi du code par WhatsApp via le NotificationService asynchrone
    NotificationService.dispatch({
      atelierId: 'atl-1787175204484',
      recipient: cleanPhone,
      event: 'ORDER_CREATED', // canal sécurisé WhatsApp
      variables: {
        clientName: 'Utilisateur',
        orderNumber: `OTP-${rawOtp}`,
        amount: 0,
        deliveryDate: 'Immédiate',
        atelierName: `Code de vérification WhatsApp : ${rawOtp} (Valide 5 minutes). Ne le partagez jamais.`
      }
    });

    console.log(`🔒 [OTP Security] Challenge généré et hashé pour ${cleanPhone} (Exp: 5 min, Rate limit OK)`);

    return {
      success: true,
      message: 'Code de vérification envoyé sur votre WhatsApp. Valide 5 minutes.'
    };
  }
